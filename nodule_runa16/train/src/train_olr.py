...
class Trainer:
    ...
    def train_epoch(self, epoch):
        ...
        for train_data, train_label, train_rlabel in self.train_loader:
            ...
            if self.opt['cuda']:
                ...
                float_scores = self.scoremat[train_label].type_as(train_label)
                float_scores = Variable(float_scores).cuda()
                train_data = Variable(train_data).cuda()
            score1 = self.net(train_data)
            ...
            # loss2 = self.regular(score2, float_scores.view(-1, 1).float())
            # self.opt['lambda'] is set to 0.
    ...
    def val_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for val_data, val_label, val_rlabel in self.val_loader:
                if self.opt['cuda']:
                    ...
                    float_scores = self.scoremat[val_label].type_as(val_label)
                    float_scores = Variable(float_scores).cuda()
                    val_data = Variable(val_data.cuda())

                score1 = self.net(val_data)
                loss = self.criterion(score1, val_label)
                _, pred_net = torch.max(score1.detach(), 1)
        ...
        prob5 = np.concatenate(prob_list, axis=0)
        prob_0 = np.sum(prob5[:,0:3], axis=1, keepdims=True)
        prob_1 = np.sum(prob5[:,3: ], axis=1, keepdims=True)
        prob = np.concatenate([prob_0, prob_1], axis=1)
        auc = metrics.roc_auc_score(label_list, prob[:,1])
