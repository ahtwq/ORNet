1. please download dataset

download link:
	IDRiD: https://idrid.grand-challenge.org/Grading/ 
	Messidor: https://www.adcis.net/en/third-party/messidor/
	LUNA16: https://luna16.grand-challenge.org/Data/


2. process images (from IDRiD and Messidor) by executing this command: bash run_gray.sh
You can also download the processed data directly from Baidu Cloud. (https://pan.baidu.com/s/1IH1Xn5C59d1ufSKyBdyFHA, extraction code: nc23)


3. move images to test_resize1024 and train_resize1024

NOTE:
The folder have some images for testing.  If you download the full dataset, you can choose to delete it.